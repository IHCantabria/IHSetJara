...
def jara(Hb_, Hcr_, xrini, dt, gamma, xc, hc, B, Ar, hbe_, xre_, pol, Vol, Ca, Ce):
    ...
    def P_evolutivo_parabolico(pol, xro, xre, C):
        a = pol[0]
        b = pol[1]
        exponent = (xro + xre + b / a) / (xro - xre) * np.exp(C * a * (2 * xre + b / a) * dt)
    ...
    for i in range(len(Hb_)-1): 
        ...
        if Hb_[i] == 0 or Hb_[i] <= Hcr_[i]:
            ...
            xr_[i] = P_evolutivo_parabolico(pol, xro, xre, Ca)
        else:  # Si se va a producir erosión
            xr_[i] = P_evolutivo_parabolico(pol, xro, xre, Ce)
